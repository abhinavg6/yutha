...
//! - **S13: Simulation harness end-to-end (Phase 3e regression
//!   guard).** Drives the `yutha_sim` harness over a refund-cap
//!   constitution with the canonical persona bundle. Locks five
//!   load-bearing properties: simulation terminates at the
//!   budget, baseline persona never gets denied, adversarial
//!   persona's escalating probe trips the cap forbid rule, the
//!   four-stage enforcement chain fires at least once, and the
//!   baseline persona is never quarantined by the same
//!   constitution that quarantines the attacker.

pub mod s10_shadow_mode;
pub mod s11_replay_session;
pub mod s12_constitution_diff;
pub mod s13_simulation_harness;
...
pub use s13_simulation_harness::{run_s13, S13Outcome};
