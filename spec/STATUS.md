...
Implementation phase for the OIDC Attestor — the broad-compatibility enterprise `Attestor` backend introduced as a sketch in [RFC 0016 §3.6](./rfcs/0016-attestor-interface.md#36-reference-impl-sketch--oidc-phase-f) and defined byte-exactly in [`/spec/identity-keys/attestor-oidc.md`](./identity-keys/attestor-oidc.md). Lands the crate + the verify path + the JSON conformance vectors + the operator runbook. The OIDC Attestor is now feature-complete alongside SPIFFE (Phase E); the enterprise-identity walkthrough (Phase G) remains ahead.

| Artifact | Status |
|----------|--------|
| [`/spec/identity-keys/attestor-oidc.md`](./identity-keys/attestor-oidc.md) | Byte-exact verification contract: ID-token format pinning (RS256/RS384/RS512/ES256/ES384/EdDSA — HS* + `none` architecturally rejected), 9-step verification algorithm with ordering rationale, three JWKS sources (OIDC Discovery / `jwks_uri` override / static file), bounded-staleness policy + kid-miss async refresh + 64 KiB payload cap, iss/aud binding + discovery-doc `issuer` exact-match per RFC 8414 §3.3, `oidc:<iss>:<sub>` external_identity composition, operator-allowlisted claim projection with caps, error-mapping table, CLI surface, conformance vector layout, threat-model deltas vs NativeAttestor. |
| [`crates/yutha-attestor-oidc/`](../crates/yutha-attestor-oidc/) | Shipped. `OidcAttestor` impl + `OidcConfig` + `JwksSource::{Discovery, JwksUri, StaticFile}` + `JwksCache` + `map_oidc_error` + payload projection. Built on `jwks ^0.5` (chenhunghan — JWKS parse + OIDC discovery shortcut) + `jsonwebtoken ^10` (offline JWT verify + ErrorKind drill-down for error mapping). The cache is in-tree: TTL refresh via lazy `assert_fresh` + background `tokio::spawn`; kid-miss via blocking `refresh_now` with before/after `last_refresh_at` snapshot for proper concurrent-caller dedup (a kid-rotation regression-guard test in `tests/integration.rs` catches the wrong-heuristic bug class). |
| [`crates/yutha-control-plane/src/main.rs`](../crates/yutha-control-plane/src/main.rs) | `AttestorArg::Oidc::build` now constructs a real `OidcAttestor` (replacing Phase E's `bail!("lands in Phase F")` placeholder). 11 new CLI flags: `--attestor-oidc-{issuer,audience,jwks-uri,jwks-file,project-claims,allowed-algs,cache-ttl-secs,max-staleness-secs,clock-skew-secs,connect-timeout-secs,allow-insecure-http}`, each with `YUTHA_ATTESTOR_OIDC_*` env-var equivalent. Validation: issuer + audience required + at most one of jwks-uri/jwks-file (default → Discovery); HS*/none rejected at config-validate time; HTTPS enforced unless `--allow-insecure-http`. |
| [`/spec/vectors/attestor/oidc/`](./vectors/attestor/oidc/) | 7 JSON conformance fixtures committed across 6 subdirectories (accept-es256 happy path + projected claims, reject-issuer, reject-audience, reject-expired, reject-signature bit-flipped, reject-empty). Deterministic regen at `crates/yutha-attestor-oidc/tests/regen_vectors.rs` (`#[ignore]`-gated, ECDSA-P256 keypair from a ChaCha20-seeded RNG, `PSEUDO_NOW = 2099-01-01Z` so accept fixtures stay valid for decades, hardcoded early-2000s timestamp for the expired case per the time-relative-fixtures memory entry). Cross-impl JSON contract for any OIDC Attestor reimplementation. **Deliberate v1 deviation** from spec §11's 25-case target — see the vectors README for rationale. |
| [`crates/yutha-attestor-oidc/tests/`](../crates/yutha-attestor-oidc/tests/) | Four test files: `forged_jwts.rs` (10 cases — RS256/ES256/EdDSA happy paths + 5 claim-failure rows needing valid sigs + claim projection + aud-array), `integration.rs` (4 cases — axum-based mock OIDC server, Discovery + JWKS-URI + issuer-mismatch reject + kid-rotation; runs in CI without `#[ignore]`), `vectors.rs` (loader iterating every `*.json` under the spec dir), `common/mod.rs` (shared `SigningFixture` for RSA-2048 / ECDSA-P256 / Ed25519 + JWT mint helpers). |
| [`docs/operator/oidc-attestor.md`](../docs/operator/oidc-attestor.md) | Operator runbook in the Phase E SPIFFE-runbook tradition: source-decision table (Discovery / JWKS-URI / static-file), audience selection guidance, per-IdP recipes (Auth0 / Okta / Azure AD / Keycloak / Google Workspace), CLI configuration walkthrough for each mode, freshness/skew tuning, HTTPS escape-hatch posture, client-side wiring example (Python SDK threading `external_credential=`), verification + receipt evidence shape, 12-row troubleshooting table, docker-keycloak local-developer recipe, security posture vs A1/A6/A8 adversaries. Wired into mkdocs nav as "OIDC Attestor (identity)" between the SPIFFE runbook and Monitoring. |

Verification gates that passed before declaring Phase F done: `cargo build --workspace`, `cargo test --workspace`, `cargo clippy --workspace --all-targets -- -D warnings`, `cargo deny check`, all in-tree forged-JWT + JSON-vectors tests, `mkdocs --strict`. The in-process mock-OIDC server (axum 0.8) lets the integration tests run end-to-end in CI without docker.

Phasing checkpoint: Phase F ends here. Phase G (`docs/operator/enterprise-identity.md` end-to-end walkthrough — combining SPIFFE/OIDC + a cloud-KMS signer in one operator-friendly narrative) is the only remaining workstream step. Local commit lands here; per the per-phase-commit-end-of-workstream-push posture, `git push` waits until Phase G ships.
