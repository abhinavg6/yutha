...
## Domain: Replay (Phase 3c)

Replay-session lifecycle receipts land in the **production** receipt store as operator audit records. Within-session evaluation receipts (`constitution.evaluate.{pass,deny}`, `envelope.send`, `enforcement.*`, etc.) land in the session-scoped replay store and carry an additional `replay_session_id` evidence entry. Per RFC 0018 §4.3, within-session receipts reuse the same canonical action-kind strings as production; store membership plus the `replay_session_id` evidence marker distinguishes them. Per RFC 0018 §4.4, the `AnchorDriver` is bound to the production store only — replay receipts NEVER anchor to Sui.

| `action_kind` | Producer | Actor | Notes |
|---------------|----------|-------|-------|
| `replay.session.create` | Replay engine | Operator | An operator created a replay session (RFC 0018 §4.5). Lands in the **production** receipt store. Evidence: `replay_session_id` (UUIDv7), `candidate_constitution_hash`, `candidate_constitution_version`, `receipt_window_from_unix_ns`, `receipt_window_to_unix_ns`, `action_kind_filter` (comma-joined string; empty = wildcard), `replay_mode` (`"cold"` \| `"warm"`), `warm_lookback_hours` (only present when `replay_mode == "warm"`). |
| `replay.session.close` | Replay engine | Operator | A replay session has been closed — explicitly via `ReplayService.CloseSession` or automatically after the inactivity TTL (default 24 hours). Lands in the **production** receipt store. Evidence: `replay_session_id`, `receipts_replayed_total` (u64), `close_reason` (`"explicit"` \| `"ttl"`), `session_create_receipt_id` (content-address of the corresponding `replay.session.create` receipt — direct join key for the lifecycle pair). |
