...
A Yutha swarm is built from exactly four primitives — passport, envelope, receipt, capability. Not three, not seven. The number is a design choice: three would force one of identity, authority, or evidence to ride on top of the others (always a leaky abstraction); seven would mean the substrate is reasoning about things the application should own. Four is the smallest set that lets you talk honestly about *who* an agent is, *what* it's allowed to do, *what* it actually did, and *how* it said the thing it said. Everything else in the system — the constitution, the four-stage enforcement loop, the verifiable anchor, the framework adapters — is a composition over these four. If you understand them, the rest of Yutha follows.

This page is the conceptual tour. It tells you what each primitive is, why it exists, what mental model to load it with, and how the four compose into a useful flow. For the formal wire format, field-by-field rationale, and conformance hooks, the `/spec/` directories are authoritative and linked at the end of each section. The threat model that shaped these primitives — hostile agent (A1), compromised model (A2), prompt injection (A3), network adversary (A5), sybil (A6), malicious operator (A8), compromised supervisor (A9) — is what each design choice is defending against; if you see a constraint that looks unnecessarily strict, it's usually because one of those adversaries is the reason.
